...
type t = { id : id; name : string } [@@deriving yojson]

let from_tuple (id, name) = { id; name }

let list =
  let query =
    let open Caqti_request.Infix in
    T.(unit ->* tup2 int string) "SELECT rowid, name FROM project"
  in
  fun (module Db : DB) ->
...
let find =
  let query =
    ...
    T.(int ->! tup2 int string)
      "SELECT rowid, name FROM project WHERE rowid = $1"
  in
  fun id (module Db : DB) ->
