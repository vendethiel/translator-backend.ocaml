...
module Translations = struct
  ...
  let yojson_of_map xs =
    `Assoc (List.map (function k, v -> (k, `String v)) @@ bindings xs)
  ...
  let map_of_yojson = function
    | `Assoc xs ->
        ...
        @@ List.map
             (function
               | k, `String v -> (k, v)
               | _ -> raise @@ WrongTranslationFormat "Expected a string value")
             xs
...
let from_tuple (id, project_id, name, translations) =
  {
    ...
    name;
    translations =
      translations |> Yojson.Safe.from_string |> Translations.map_of_yojson;
  }
...
let list =
  let query =
    ...
    T.(int ->* tup4 int int string string)
      "SELECT rowid, project_id, name, translations FROM task WHERE project_id \
       = $1"
  in
...
let translate =
  let query =
    ...
    T.(tup3 string string int ->. unit)
      (* we'll have to make sure the key is a simple key and can't be torn down further *)
      "UPDATE task SET translations = json_set(translations, '$.' || $1, $2) \
       WHERE rowid = $3"
  in
